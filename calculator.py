...
def show_calculator(frame, calculator):
    ...
    explanation_part2 = tk.Label(frame, text="each task and press calculate.", font='lucinda 11 bold',
                                 bg='Pink', fg='Black')
    explanation_part2.place(x=30, y=100)

    # These are input fields for the hours spent on each task of your day to calculate later.

    scale_work = tk.Scale(frame, from_=0, to=168, orient="horizontal", length=240)
    scale_work.place(x=150, y=130)
    work_label = tk.Label(frame, text="WORK", font='lucinda 11 bold', fg='Black')
    work_label.place(x=30, y=130)

    scale_sleep = tk.Scale(frame, from_=0, to=168, orient="horizontal", length=240)
    scale_sleep.place(x=150, y=180)
    sleep_label = tk.Label(frame, text="SLEEP", font='lucinda 11 bold', fg='Black')
    sleep_label.place(x=30, y=180)

    scale_carework = tk.Scale(frame, from_=0, to=168, orient="horizontal", length=240)
    scale_carework.place(x=150, y=230)
    carework_label = tk.Label(frame, text="CARE WORK", font='lucinda 11 bold', fg='Black')
    carework_label.place(x=30, y=230)

    scale_freetime = tk.Scale(frame, from_=0, to=168, orient="horizontal", length=240)
    scale_freetime.place(x=150, y=280)
    freetime_label = tk.Label(frame, text="FREE TIME", font='lucinda 11 bold', fg='Black')
    freetime_label.place(x=30, y=280)

    # This creates the calculate button.

    calculate_button = tk.Button(frame, text="CALCULATE", font='lucinda 15 bold', height=1, width=15,
                                 command=lambda: calculate_worklifebalance(frame, scale_work, scale_sleep, scale_carework, scale_freetime, calculator))
    calculate_button.place(x=30, y=330)

def calculate_worklifebalance(frame_calculator, input_worktime, input_sleeptime, input_careworktime, input_freetime, calculator):
    ...
    show_piechart(frame_calculator, (input_worktime.get()+1)/sum_hours, (input_sleeptime.get()+1)/sum_hours, (input_careworktime.get()+1)/sum_hours, (input_freetime.get()+1)/sum_hours)
